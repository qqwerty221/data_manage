...
import {
  ...
  value_default,
  zoom_default
} from "./chunk-OX5ZBIQY.js";
import {
  now,
  timeout_default,
  timer
} from "./chunk-TT5QQZE3.js";
...
function bisector(f) {
  ...
  function left2(a3, x2, lo = 0, hi = a3.length) {
    if (lo < hi) {
      ...
      do {
        const mid = lo + hi >>> 1;
        if (compare2(a3[mid], x2) < 0) lo = mid + 1;
        else hi = mid;
      } while (lo < hi);
    }
    return lo;
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold2 = constant_default5(20);
  function grid(data) {
  ...
  density.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default5(+_), density) : x2;
  };
  density.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default5(+_), density) : y2;
  };
...
function orient2dadapt(ax, ay, bx, by, cx, cy, detsum) {
  ...
  let bvirt, c5, ahi, alo, bhi, blo, _i, _j, _0, s1, s0, t12, t02, u32;
  const acx = ax - cx;
  const bcx = bx - cx;
  const acy = ay - cy;
  const bcy = by - cy;
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x2, y2, c5 = c0 || c1;
      if (c5 & 8) x2 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y2 = this.ymax;
      else if (c5 & 4) x2 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y2 = this.ymin;
      else if (c5 & 2) y2 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x2 = this.xmax;
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c5 = (this.ymin - y06) / vy) < t) y2 = this.ymin, x2 = x06 + (t = c5) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c5 = (this.ymax - y06) / vy) < t) y2 = this.ymax, x2 = x06 + (t = c5) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c5 = (this.xmax - x06) / vx) < t) x2 = this.xmax, y2 = y06 + (t = c5) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c5 = (this.xmin - x06) / vx) < t) x2 = this.xmin, y2 = y06 + (t = c5) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x2, y2, i = 0) {
    ...
    const i0 = i;
    let c5;
    while ((c5 = this._step(i, x2, y2)) >= 0 && c5 !== i && c5 !== i0) i = c5;
    return c5;
  }
  _step(i, x2, y2) {
...
var halfPi2 = pi3 / 2;
function sinIn(t) {
  return +t === 1 ? 1 : 1 - Math.cos(t * halfPi2);
}
function sinOut(t) {
  return Math.sin(t * halfPi2);
}
function sinInOut(t) {
...
function interpolate_default2(a3, b) {
  var x06 = a3[0] * radians, y06 = a3[1] * radians, x12 = b[0] * radians, y12 = b[1] * radians, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin2(d);
  var interpolate = d ? function(t) {
    var B2 = sin2(t *= d) / k2, A = sin2(d - t) / k2, x2 = A * kx0 + B2 * kx1, y2 = A * ky0 + B2 * ky1, z = A * sy0 + B2 * sy1;
    return [
      atan2(y2, x2) * degrees,
      atan2(z, sqrt(x2 * x2 + y2 * y2)) * degrees
    ];
  } : function() {
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b02 + b12, c5 = c0 + c1, m2 = sqrt(a3 * a3 + b * b + c5 * c5), phi2 = asin(c5 /= m2), lambda22 = abs3(abs3(c5) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
function conicConformalRaw(y06, y12) {
  ...
  function project(x2, y2) {
    if (f > 0) {
      if (y2 < -halfPi3 + epsilon6) y2 = -halfPi3 + epsilon6;
    } else {
      if (y2 > halfPi3 - epsilon6) y2 = halfPi3 - epsilon6;
    }
...
function pack_default() {
  ...
  pack.padding = function(x2) {
    return arguments.length ? (padding = typeof x2 === "function" ? x2 : constant_default7(+x2), pack) : padding;
  };
  return pack;
}
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x2 = 1, s2 = 1, t02, t12, t2, k10, k21, interpolator = identity2, transform2, clamp = false, unknown;
  function scale2(x3) {
  ...
  scale2.domain = function(_) {
    return arguments.length ? ([x06, x12, x2] = _, t02 = transform2(x06 = +x06), t12 = transform2(x12 = +x12), t2 = transform2(x2 = +x2), k10 = t02 === t12 ? 0 : 0.5 / (t12 - t02), k21 = t12 === t2 ? 0 : 0.5 / (t2 - t12), s2 = t12 < t02 ? -1 : 1, scale2) : [x06, x12, x2];
  };
  scale2.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale2) : clamp;
  };
  ...
  return function(t) {
    transform2 = t, t02 = t(x06), t12 = t(x12), t2 = t(x2), k10 = t02 === t12 ? 0 : 0.5 / (t12 - t02), k21 = t12 === t2 ? 0 : 0.5 / (t2 - t12), s2 = t12 < t02 ? -1 : 1;
    return scale2;
  };
}
function diverging() {
...
function line_default2(x2, y2) {
  ...
  x2 = typeof x2 === "function" ? x2 : x2 === void 0 ? x : constant_default8(x2);
  y2 = typeof y2 === "function" ? y2 : y2 === void 0 ? y : constant_default8(y2);
  function line(data) {
  ...
  line.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default8(+_), line) : x2;
  };
  line.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default8(+_), line) : y2;
  };
...
function area_default5(x06, y06, y12) {
  ...
  x06 = typeof x06 === "function" ? x06 : x06 === void 0 ? x : constant_default8(+x06);
  y06 = typeof y06 === "function" ? y06 : y06 === void 0 ? constant_default8(0) : constant_default8(+y06);
  y12 = typeof y12 === "function" ? y12 : y12 === void 0 ? y : constant_default8(+y12);
  function area(data) {
...
function link2(curve) {
  ...
  link3.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default8(+_), link3) : x2;
  };
  link3.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default8(+_), link3) : y2;
  };
